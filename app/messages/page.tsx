...
function MessagesContent() {
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    await supabase.from('messages').insert({ conversation_id: activeConvo.id, sender_id: user.id, content: newMessage.trim() });
    setNewMessage('');
      ...
      <div className="flex-1 flex max-w-5xl mx-auto w-full">
        <div className={`w-full md:w-80 border-r bg-white ${activeConvo ? 'hidden md:block' : ''}`}>
          {loading ? (
            ...
            <div className="text-center py-12 px-4"><p className="text-gray-500">No conversations yet</p></div>
          ) : (
              ...
              {conversations.map(convo => {
                ...
                return (
                  <button key={convo.id} onClick={() => setActiveConvo(convo)} className={`w-full p-4 text-left hover:bg-gray-50 flex items-center gap-3 ${activeConvo?.id === convo.id ? 'bg-blue-50' : ''}`}>
                    <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold flex-shrink-0">{other.full_name?.[0] || '?'}</div>
          ...
          {activeConvo ? (
            <>
              ...
              <form onSubmit={sendMessage} className="bg-white border-t p-3 flex gap-2">
                <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type a message..." className="flex-1 border rounded-full px-4 py-2 text-sm" />
                <button type="submit" className="bg-blue-600 text-white rounded-full px-5 py-2 text-sm font-medium">Send</button>
              </form>
            </>
          ) : (
